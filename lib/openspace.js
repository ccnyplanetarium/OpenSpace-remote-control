...
class openspace {
  ...
  constructor(setting) {
    ...
    if(setting['config']['OpenSpaceAutoRunPath']) {
      var child_process = require('child_process');
      child_process.exec("start cmd.exe /K "+setting['config']['OpenSpaceAutoRunPath']);
      console.log("cmd.exe /K "+setting['config']['OpenSpaceAutoRunPath'])
    }
  ...
  */
  screenshot(setting) {
    ...
    */
    ...
    /**
    * Function to get the api value

    * @param {object} event The api event
    * @param {function} receivingCallback The callback event

    */
    get(event,receivingCallback) {
      const callback = () => {
        // https://www.linode.com/docs/development/introduction-to-websockets/
        this.ws.onmessage = function(e) {
          return receivingCallback(JSON.parse(e.data));
        };
      };
      this.ws.send(JSON.stringify(
        {
